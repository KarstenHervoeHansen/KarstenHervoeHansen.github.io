...
include("rmconfig.php");
	...
	{ 
		...
		if (ValidateEndTime($serial) == true) {
			 $count = 1; ;
		     WriteLog('idleloop ValidateEndTime','true');
		}
		return $count;
	}
		 ...
		 )
	{
		...
		WriteLog('idleloop SWAResult',$SWAResult);
		WriteLog('idleloop GetUpdateInfo',GetUpdateInfo($deviceid, $packageid));		
	}
	else
	{
		WriteLog('idleloop deviceid',$deviceid);
	    WriteLog('idleloop serial',$serial);
		WriteLog('idleloop','fejl');
